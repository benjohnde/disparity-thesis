...
The following sections accurately describe the implementation, its components and the subsequent evaluation pipeline for disparity maps.
Chapter \ref{chap:related} points out that no real algorithm for stereoscopic video disparity yet exists.
Also, no evaluation suite that assesses disparity algorithms based on stereoscopic videos has not been build yet.
...
\newline\newline\noindent Thus, the decision towards an implementation of a novel evaluation suite, built on top of OpenCV, was made.
...
As a timer saver and for reducing code duplicates OpenCV was used.
...
\begin{figure}[h!]
  ...
  \end{tikzpicture}
  \caption[Illustration of the evaluation engine]{Illustration of the evaluation engine.}
...
Though, this approach would produce erroneous results due to a variety of reasons.
...
\end{tabular}
\caption[Frame of the SVDDD dataset and the corresponding salient regions mask]{Frame of the field sequence from the SVDDD dataset and the corresponding salient regions mask from the static saliency approach.}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    \begin{axis}[every axis plot post/.append style={
      ...
      axis y line*=left,
      enlargelimits=upper]
      \addplot {gauss(0,3.16228)};
    \end{axis}
  \end{tikzpicture}
  \caption[Gaussian normal distribution]{Gaussian normal distribution with $\mathcal{N}(0,\sqrt{10})$.}
  \label{fig:gaussian}
\end{figure}
...
\newline\newline\noindent Algorithm \ref{alg:dsi-3d}, \textsc{CreateDisparitySpaceImage}, calculates the matching cost via Algorithm \ref{alg:matchingcost}, \textsc{MatchingCost} for each possible disparity and stores them in a four-dimensional matrix.
As input parameter, the Algorithm \ref{alg:dsi-3d}, \textsc{CreateDisparitySpaceImage}, expect a set of rectified stereo image pairs, $I_L=\{I_{L_{1}}...I_{L_{n}}\}$ and $I_R=\{I_{R_{1}}...I_{R_{n}}\}$, whereas $n$ denotes the amount of images.
...
\textsc{CreateMatrix} creates a matrix with dimensions according to the amount of input parameters.
...
Algorithm \ref{alg:disparity-map}, \textsc{DisparityMap}, then computes the final disparity map by taking the disparity with the minimum matching cost for each field in the reference matrix.
\textsc{DisparityMap} needs as parameter the disparity space image, here denoted as $C(x,y,t,d)$ and the number of the frame $f$, for which the disparity map should be computed.

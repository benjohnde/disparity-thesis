...
\begin{itemize}
  ...
  \item on the other hand, those algorithms have some troubles with the constructed scenery.
\end{itemize}
...
\noindent Without removing these elements in a scene, a stereo matching algorithm may have problems with those arbitrary textures.
Also, obtaining depth information from Blender can be problematic, if transparent layers are in between the actual background of the scene, which is visible for the viewer, and the camera.
This can lead to wrong depth information and later on to false disparity information.
...
\newline\newline\noindent The above mentioned points apply to the first alpha version of the dataset.

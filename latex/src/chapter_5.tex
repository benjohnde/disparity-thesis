...
%wollte deine meinung wissen ob du findest, dass diese evaluation eine geschichte erzählt:
%
%- zunächst stelle ich die datasets kurz vor
%- danach beschreibe ich meine werkzeuge (metrics) zum messen der performance der algorithmen
%- anschließend wie das visualiert wird (mit den heatmaps) und welche resultate es gibt (zum beispiel kombination von masken mit den metriken)
%- dann beschreibe ich noch kurz die ausführung via scripts und parameter-tuning (also ob und was ich getan habe)
%
%danach erfolgen folgende messungen:
%- gegen ein reference dataset (wo ich resultate von ähnlichen algorithmen habe) um zu schauen, ob das total aus dem ruder läuft oder eben halbwegs plausible ergebnisse liefert
%- anschließend evaluiere ich mit einem video-dataset folgendes: alle algorithmen drüber laufen lassen, dann verschiedene gaussian noise stufen, dann verschiedene stufen der video komprimierung (um zu schauen ob die artefakte was ausmachen)
%- davon getrennt evaluiere ich dann auf das dataset meinen eigenen stereo matcher: spatiotemporal-context vs non-spatiotemporal-context um zu schauen was das ändert
%- zum schluss evaluiere ich mit zwei algorithmen noch das SVDDD dataset vom lehrstuhl (bzw ein paar szenen welche stephan gerade noch abändert). warum ich nur zwei algorithmen nehme erkläre ich dann noch.
%- anschließend kurzes wrap-up

...
That means, that the previously introduced disparity constraints are not handled properly.
\newline\newline\noindent Algorithm \ref{alg:dsi-3d}, \textsc{CreateDisparitySpaceImage}, calculates the matching cost via Algorithm \ref{alg:matchingcost}, \textsc{MatchingCost} for each possible disparity and stores them in a three-dimensional matrix.
...
\textsc{CreateMatrix} creates a matrix with dimensions according to the amount of input parameters and the given size of each parameter.
...
$C(x,y,d,t)$ is then constructed in Algorithm \ref{alg:dsi-3d} whereby $t \in T=\{0{\dots}n\}$ denotes the current processed frame.
...
This approach is reflected by the parameter $weighted$ in Algorithm \ref{alg:disparity-map}, which can be \textsc{True} or \textsc{False}.

...
\newline\newline\noindent The Algorithm \ref{alg:dsi-3d}, \textsc{CreateDisparitySpaceImage}, calculates the matching cost via Algorithm \ref{alg:matchingcost}, \textsc{MatchingCost} for each possible disparity and stores them in a three-dimensional matrix.
As input parameter, the Algorithm \ref{alg:dsi-3d}, \textsc{CreateDisparitySpaceImage}, expect a pair of rectified stereo images, $I_L$, $I_R$, the maximum disparity to be found $d_{max}$ and $wSize$ as window size for the blocks to be matched.
\textsc{Rect}\{x,y,width,height\} denotes a rectangle cropped out of a given image.
Furthermore, the matching cost can be queried with the function $C(x,y,d)$, which also represents the disparity space image.
...
$\textsc{Disparities}(C)$ outputs $d_{max}$, the length of the array in this matrix.
...
\newline\newline\noindent None the less, it is exciting to see, if the naive spatiotemporal approach achieves better results than without.

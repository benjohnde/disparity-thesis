...
\tikzstyle{rblock} = [text width=9em, rectangle, draw, fill=gray!25, text centered, rounded corners, minimum height=4em]
\tikzstyle{cloud} = [text width=6em, ellipse, draw, fill=gray!10, text centered, rounded corners, minimum height=3.2em]
\begin{figure}[h!]
  \centering
  \begin{tikzpicture}[node distance=5.2em, auto]
    %input
    ...
    %computation
    \node [rblock, below of=right, xshift=3em] (exec) {(1) Algorithm \\\quad executer};
    \node [rblock, below of=disp, xshift=-3em] (mask-creator) {(2) Mask creator};
    ...
    %evaluation
    \node [rblock, below of=left, yshift=-10em] (evaluation) {(3) Disparity \\\quad evaluator};

...
\begin{frame}[fragile]{Matching cost}
  ...
  \end{itemize}
  \visible<2->{
    \begin{exampleblock}{Sum of absolute differences}
      $\displaystyle \operatorname{SAD} = \sum_{i,j \in U} \left| I_1(x_L+i,y_L+j) - I_2(x_R+i,y_R+j) \right|$
    \end{exampleblock}
  }
\end{frame}
...
\begin{frame}[fragile]{Simple block-matching}
  \begin{figure}[h!]
    \centering
...
\begin{frame}[fragile]{Spatiotemporal stereo matcher}
  \scalebox{0.8}{
    \begin{algorithm}[H]
    \DontPrintSemicolon
...
\begin{frame}[fragile]{Masking modes}
  \begin{columns}[T,onlytextwidth]
    ...
    \only<2>{
      ...
      \caption{Ground-truth companion}
      \end{figure}
    }
    \only<3>{
      \begin{figure}
...
\begin{frame}[fragile]{Results}
  ...
  \caption{Result table for comparison of own implementation}
  \end{table}
\end{frame}
